...
export function Calc(carInfo, submitDateString) {
    ...
    if (carInfo.BigFixed || carInfo.PersonalService) {
        var maintenanceMonths = [purchusMonth % 12, (purchusMonth + 3) % 12, (purchusMonth + 6) % 12, (purchusMonth + 9) % 12].map(month => {
            return month = 0 ? 12 : month;
        });

        if (maintenanceMonths.some(m => m - submitMonth == 1 || m - submitMonth == 0)) {
            return carInfo;
        }
    } else {
